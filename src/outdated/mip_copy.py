...
def run_optimisation(file_number : int, P : float, 
    ...
    # V = set(v for v in G.V if v != 0)
    # A = set(e for e in G.edges if e[0] != 0)
    V = G.V
    A = G.edges
    ...
    Outgoing = { # stores nodes that go out of j
        j : [k for k in V if (j, k) in A]
        for j in V
    }

    Incoming = { # stores node that go into j
        j : [k for k in V if (k, j) in A] for j in V
    }
    ...
    for Threshold in [0.9,0.8,0.7,0.6]:
        ...
        for a in X:
            if X[a].LB < Threshold and X[a].X > Threshold:
                X[a].LB = 1
                numFixed += 1
        if numFixed == 0:
            break
    ...
    # for i, j in F:
    #     RHS = round(Theta[j] + sum(F[j, k].X for k in V if k in Outgoing[j]) - M*X[i, j].X, 5)
    #     LHS = round(F[i, j].X, 5)
    #     print(i, j, LHS, RHS, LHS == RHS)
    return m.ObjVal
...
if __name__ == "__main__":
    ...
    import time
    t1 = time.time()
    output = run_optimisation(7, P, verbal=False)
    t2 = time.time()
    print(t2 - t1)